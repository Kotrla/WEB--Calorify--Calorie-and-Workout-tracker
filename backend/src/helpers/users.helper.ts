...
import { IUserModel, getAge } from '../models/user.model.js';
import { calculateMacros } from '../utils/macroFunctions.js';
import { IUserResponse } from 'ts/models/response/users-responses.model.js';
import { IUserUpdateRegisterRequest } from 'ts/models/request/users-requests.model.js';
...
export async function generateNewUserFromRequest(req: Request<{}, IUserResponse, IUserUpdateRegisterRequest>) {
	...
	return {
		personal: { ...personal, age: calculatedAge },
		needs: { protein, carbs, fats, kcal },
		credentials: { ...credentials, password: encryptedPassword },
		stats
	};
}
